...
class Folder(models.Model):
    ...
    def __str__(self):
        return self.name
        db_table = u'"cableroutes\".\"cablesystem"'
            ...
            try:
                ...
                if alreadyUsedName is not None:
                    ...
                    while not found:
                        ...
                        try:
                            test= Folder.objects.get(name=essai,parent=self.parent,username=self.username)
                            indice = indice +1 
                        except:
                            found = True
                            self.name=essai
            ...
            try:
                ...
                if alreadyUsedName is not None:
                    ...
                    while not found:
                        essai=f"{self.name}_{indice}"    
                        try:
                            test= Map.objects.get(name=essai,folder=self.folder,username=self.username)
                            indice = indice +1 
                        except:
                            found = True
                            self.name=essai
